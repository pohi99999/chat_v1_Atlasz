...
export async function POST(req: NextRequest) {
  try {
    ...
    // Convert to LangChain format and stream
    const langChainMessages = formattedMessages.map((msg) => {
      if (msg.role === "system") {
        return { _getType: () => "system" as const, content: msg.content };
      }
      if (msg.role === "user") {
        return { _getType: () => "human" as const, content: msg.content };
      }
      return { _getType: () => "ai" as const, content: msg.content };
    });

    // Start streaming
    model.call(langChainMessages, {}, [handlers]).catch(console.error);
